...
int PosLedRelay[16] = {1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14};
...
void carga_unidades() {
  ...
  for (int i = 0; i <= 15; i++) {
    ...
    else if (corrientes[i] >= 90 &&  corrientes[i] <= 400) {
      //amarillo
      pixels.setPixelColor(PosLedRelay[i], pixels.Color(80, 255, 0));
      pixels.show();
      delay(5);
    ...
    else if (corrientes[i] <= 90 && corrientes[i] >= 6) {
      //verde
      contador ++;
...
void imprimir_Datos(unsigned char SlaveAddress) {
  switch (SlaveAddress) {
    ...
    case 0x4E: Serial.print(" 0x4E, "); Serial.print(current_mA ); delay(0); break;
